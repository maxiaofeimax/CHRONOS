...
parser.add_argument('--max_round', type=int, default=5,
                    help='Rounds of Question')
parser.add_argument('--model_name', type=str,
                    default='qwen2.5-72b-instruct', help='Model')
parser.add_argument('--dataset', type=str,
                    choices=['open', 'crisis', 't17'], default='open', help='Dataset used')
    ...
    for _ in range(2):
        try:
            ...
            if search_engine != 'bing':
                doc_list_all = search(
                    [input_text + ' timeline'], n_max_doc, search_engine)
            else:
                doc_list_all = search(
                    [input_text], n_max_doc, search_engine, freshness)
                # pprint(doc_list_all)
            ...
            while not news_timeline:
                news_timeline = generate_timeline(
                    model=model, news=input_text, docs=doc_list_all)    # generate timeline
                print(news_timeline)
            ...
            for i in range(MAX_ROUNDS):
                ...
                cnt = 0
                while question_list == []:
                    # question-based news background decomposition
                    question_list = ask_news_question(
                        model=model, news=input_text, docs=doc_list_all[-150:], questions=question_list_all, examples=question_examples)
                    cnt += 1
                    if cnt > 10:
                ...
                for question in question_list:
                    ...
                    print(question)
                    while query_gen == []:
                        query_gen = rewrite_query(
                            question, n_max_query, model=model)
                ...
                tic = time.time()
                if search_engine != 'bing':
                    doc_list = search(list(set(queries)),
                                      n_max_doc, search_engine)
                else:
                    doc_list = search(list(set(queries)),
                                      n_max_doc, search_engine, freshness)
                ...
                while not news_timeline:
                    news_timeline = generate_timeline(
                        model=model, news=input_text, docs=doc_list_curr_iter)    # generate timeline
                    print(news_timeline)
                    if not news_timeline:
                        time.sleep(5)
                        gen_cnt += 1
            ...
            save_json(question_list_all + doc_list_all, os.path.join(
                f'{args.output}/docs', f"{input_text.replace(' ', '_').replace('/', '')}-{model}-{MAX_ROUNDS}.json"))
...
def evaluate(dataset, model='gpt-3.5-turbo'):
    ...
    for keyword, query, index in tqdm(TARGET_KEYWORDS[dataset]):
        ...
        ground_truth = TilseGroundTruth(
            [TilseTimeline(g) for g in gt_timelines])
        num_dates = len(ground_truth.get_dates())

        begin_time = time.time()
        if dataset in ['crisis', 't17']:
            predicts = generate(
                query, model, num_dates=num_dates, search_engine='serper')
        else:
            predicts = generate(query, model, num_dates=num_dates, search_engine='serper',
                                freshness=keyword.split('_')[-1].replace('.', '-'))
        ...
        sorted_pred_timeline = dict(
            sorted(pred_timeline.items(), key=lambda item: len(item[1]), reverse=True))

        if MAX_ROUNDS > 1:
            try:
                predicts = []
                predicts = merge_timeline(
                    model=model, news=query, num_dates=num_dates, timelines=pred_timeline)
            except:
                pass
            print(predicts)
        ...
        for s, e in pred_timeline.items():
            save_timeline.append(
                {'start': s.strftime('%Y-%m-%d'), 'events': e})
        ...
        try:
            ...
            if keyword not in overall_results:
                overall_results[keyword] = {
                    "rouge": rouge_scores, "date_score": date_scores, "predict-timeline": save_timeline}
                save_json(overall_results[keyword], os.path.join(
                    f'{args.output}/timelines', f"{dataset}-{model}-{keyword}-{MAX_ROUNDS}.json"))
        except:
            print("=======Evaluation Error=======")
            pprint(save_timeline)

    avg_res = get_average_results(results)
    save_json({'res': avg_res, 'time': time_all / 3600},
              os.path.join(args.output, f"{dataset}-{model}-{MAX_ROUNDS}-avg_score.json"))
    pprint(avg_res)
    return avg_res
